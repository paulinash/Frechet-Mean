function vdp_frechet_mean_tracking()
% Visualize two Van der Pol solutions and their Fréchet mean step by step

clear; close all; clc;

%% Parameters
epsilon = 0.1;                  
a_vals = linspace(-0.9, 0.9, 3); 
Tfinal = 400;                   
dt = 0.01;                      
...
hCurves = gobjects(length(a_vals),1);
hMean = plot(NaN,NaN,'k','LineWidth',3); % placeholder for mean

axis equal
xlabel('x'); ylabel('y');
title('Van der Pol solutions and Fréchet mean - step by step');

% Plot entire trajectories in faint color for reference
for k = 1:length(a_vals)
...
for k = 1:length(a_vals)
    points(k) = plot(curves{k}(1,1), curves{k}(1,2),'o','MarkerFaceColor',colors(k,:), 'MarkerEdgeColor','k','MarkerSize',8);
end
points(end) = plot(meanCurve(1,1), meanCurve(1,2),'o','MarkerFaceColor','k','MarkerEdgeColor','k','MarkerSize',8);
