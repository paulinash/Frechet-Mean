...
%% ================ SPEED PLOTS ==================
% Speed vs reconstructed time over one mean period
% Map each sample's speed profile to its reconstructed time vector.
% Note: cumTime{k} has length M+1, speeds have length M. Use cumTime{k}(1:end-1).
figure('Color','w','Position',[100 100 1400 600]); hold on;
for k = 1:N
    tk = cumTime{k}(1:end-1);
    h = plot(tk, allSpeeds(k,:), 'Color', colors(k,:), ...
        'DisplayName', sprintf('I=%.2f', I_vals(k)));
    h.Color(4) = 0.35;
end
tmean = cumTime_mean(1:end-1);
plot(tmean, meanSp, 'k', 'LineWidth', 2.0, 'DisplayName', 'mean speed');
xlabel('reconstructed time t (one burst)');
ylabel('speed $\|\dot{\mathbf z}\|$','Interpreter','latex');
title('Speed profiles vs reconstructed time');

if export_plots
    exportgraphics(gcf,'Figures_ml/speed_vs_time.pdf','ContentType','vector');
end



%% ================= 3D PLOT WITH TRACKERS =================
fig3D = figure('Color','w','Position',[200 100 900 700]); hold on; grid on;
...
title(sprintf('3D trajectories with %d spikes', spikeCounts(1)))
view([-30 20]);
if export_plots
